...
function installErrorOnPrivate(global) {
  ...
  for (const functionName of Object.keys(disabledGlobals)) {
    global[functionName] = () => {
      throwAtFunction(disabledGlobals[functionName], global[functionName]);
    };
  }
  for (const methodName of Object.keys(disabledJasmineMethods)) {
      ...
      if ((0, _jestUtil.isPromise)(returnValue)) {
        returnValue.then(done.bind(null, null), error => {
          ...
          if (message) {
            ...
            extraError.stack = originalExtraErrorStack && originalExtraErrorStack.replace('Error: ', `Error: ${message}`);
          }
...
function makeConcurrent(originalFn, env, mutex) {
  ...
  const failing = () => {
    throw new Error('Jest: `failing` tests are only supported in `jest-circus`.');
  };
  failing.each = () => {
    throw new Error('Jest: `failing` tests are only supported in `jest-circus`.');
  };
...
var _jestUtil = require("jest-util");
var _each = _interopRequireDefault(__webpack_require__("./src/each.ts"));
var _errorOnPrivate = __webpack_require__("./src/errorOnPrivate.ts");
var _jasmineAsyncInstall = _interopRequireDefault(__webpack_require__("./src/jasmineAsyncInstall.ts"));
var _reporter = _interopRequireDefault(__webpack_require__("./src/reporter.ts"));
function _interopRequireDefault(e) { return e && e.__esModule ? e : { default: e }; }
...
async function jasmine2(globalConfig, config, environment, runtime, testPath) {
  ...
  runtime.requireInternalModule(require.resolve('./jestExpect.js')).default({
  ...
  const snapshotState = await runtime.requireInternalModule(require.resolve('./setup_jest_globals.js')).default({
    config,
    globalConfig,
